...
public class AuthController {
    ...
    public JwtDto login(@Valid @RequestBody LoginDto loginUsuario, BindingResult bindingResult) throws Exception {
        ...
        JwtDto jwtDto = new JwtDto(jwt, userDetails.getUsername(), userDetails.getAuthorities(), userToCheck.getEmail(), userToCheck.getImage()!=null? userToCheck.getImage().getDownloadLink() : "");
        return jwtDto;
    }
    ...
    public JwtDto verificate(@RequestBody LoginDto loginDto, BindingResult bindingResult, @PathVariable(name = "code") String code) throws Exception {
        ...
        String jwt = jwtProvider.generateToken(authentication);
        UserDetails userDetails = (UserDetails)authentication.getPrincipal();
        JwtDto jwtDto = new JwtDto(jwt, userDetails.getUsername(), userDetails.getAuthorities(), userToReturn.getEmail(), userToReturn.getImage()!=null? userToReturn.getImage().getDownloadLink() : "");
        return jwtDto;
    }
}
